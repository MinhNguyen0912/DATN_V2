...
namespace DATN.Core.Repositories.Repositories.ProductEAV
{
    public class ProductEAVRepository : BaseRepository<Product_EAV>, IProductEAVRepository
    {
        ...
        {
            return _context.Product_EAVs.Where(p=>p.ProductId==id).Include(p => p.Variants).ThenInclude(p => p.VariantAttributes).ThenInclude(p=>p.AttributeValue).ThenInclude(p=>p.Attribute).Include(p=>p.Brand).Include(p=>p.Origin).Include(p=>p.Images).Include(p=>p.Variants).ThenInclude(p=>p.Specifications).Include(p=>p.CategoryProducts).FirstOrDefault();
        }
